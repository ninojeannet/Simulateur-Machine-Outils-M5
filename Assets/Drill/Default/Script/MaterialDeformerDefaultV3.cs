...
namespace Simulation
{
    ...
    /// </summary>
    public class MaterialDeformerDefaultV3 : MonoBehaviour
    {
        /// <summary>
        ...
        private void EditTerrainCube(Vector3 point, float deformSpeed, float range)
        {
            ...
            float rangeXZ = boxCollider.size.x * intScaleX;
            float rangeY = boxCollider.size.y * intScaleY;
            ...
            for (int x = -intRange * intScaleX; x <= intRange * intScaleX; x++)
            {
                for (int y = -intRange * intScaleY; y <= intRange * intScaleY; y++)
                {
                    for (int z = -intRange * intScaleZ; z <= intRange * intScaleZ; z++)
                    {
                        ...
                        float distance = math.distance(new float2(offsetX, offsetZ), new float2(point.x, point.z));
                        ...
                        if (math.distance(point.y, offsetY) > rangeY)
                            continue;
